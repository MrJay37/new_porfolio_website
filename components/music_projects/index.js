...
function ProjectCard(props){
    ...
    return <Link href={props.href}>
        <div className={MusicProjectStyles.projectCard} style={{flexDirection: wideScreen > 1280 ? left_alignment ? 'row': 'row-reverse' : 'column'}}>
            <img className={MusicProjectStyles.projectCardTitle} src={props.img}/>
            <div className={MusicProjectStyles.projectCardText} style={{alignItems: wideScreen > 1280 ? left_alignment ? 'flex-end': 'flex-start' : 'center'}}>
                <h3>{props.title}</h3>
                <div className={MusicProjectStyles.tagContainer} style={{flexDirection: wideScreen > 1280 ? left_alignment? 'row-reverse' : 'row' : 'row'}}>
                    {props.genres.slice(0, 4).map((genre, i) => <div className={MusicProjectStyles.tag} key={i}>{genre}</div>)}
                </div>
            </div>
        </div>
    </Link>
